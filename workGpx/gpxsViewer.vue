<template>
    <div
        class="gsvDeb">
        gpxs viewer - {{ actionNow }} ({{ srcType }}) id: {{ idEdit }}<br>
        <hr>
    </div>
    ...
    <div v-show="srcType=='waypoints'">
        ...
        <a @click="editThisOne('waypoint',data.id)" title="Edit it"><i class="fa-solid fa-pencil"></i></a>
        | 
        <a @click="removeThisOne('waypoint',data.id)" title="Remove it"><i class="fa-solid fa-trash"></i></a>
        |
        <a v-show="dataChanged"
            @click="saveThisOne('waypoint',data.id)" title="Save changes"><i class="fa-solid fa-floppy-disk"></i></a>
        |
        <a v-show="dataChanged"
            @click="resetThisOne('waypoint',data.id)" title="Cancel changes"><i class="fa-solid fa-delete-left"></i></a>
            <br>
    ...
    <div v-show="srcType=='routes'">
        ...
        <a @click="editThisOne('route',data.id)" title="Edit it"><i class="fa-solid fa-pencil"></i></a>
        | 
        <a @click="removeThisOne('route',data.id)" title="Remove it"><i class="fa-solid fa-trash"></i></a><br>
    ...
    <div v-show="srcType=='tracks'">
        ...
        <a @click="editThisOne('track',data.id)" title="Edit it"><i class="fa-solid fa-pencil"></i></a>
        | 
        <a @click="removeThisOne('track',data.id)" title="Remove it"><i class="fa-solid fa-trash"></i></a><br>
...
export default{
    data(){
        return{
            ...
            orgData:toRaw({ layer: null, data:null, srcType: '', id: -1, source_id:-1 })
        };
    },
    mounted(){
    ...
    methods:{
        ...
        onChangeValueInForm( e = '', value='',whatChange){
            ...
            if( ['lat', 'lng','lon'].indexOf( whatChange) != -1 ){
                ...
                gpxsManager.mapio.map.flyTo([this.data.lat, this.data.lon]); 
            
            } else if( whatChange == 'name'){
                console.log('  \\_ name ',this.layer);
                prop.name = this.data.name;
                
                let trP = gpxsManager.makeBindPopup( this.layer );
        ...
        setSrc0( sid ){
            ...
            for( let s of gpxsManager.$data.gpxs ){
                if( s.id == sid ){
                    this.src0 = s;
        ...
        startEdit( srcType, id ){
            this.actionNow = 'edit';
            this.srcType = srcType;
            this.idEdit = id;
            console.log('start Edit - looking for '+srcType+' .... '+id);
            this.data = -1;
            for( let g of gpxsManager.$data.gpxs ){
            ...
            this.layer = toRaw(gpxsManager.findLayerInJGeo( srcT, id )[0]);
            this.orgData.layer = toRaw( this.layer );
